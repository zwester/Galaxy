...
namespace WindowsGame1
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        {
            /*Screen sections
             *0_tableauOne|_1_tableauTwo 
             *2_tableauThree|_3_allActions
             *4_myPhase|_5_myTableau
             *6_myHand|_7_myActions
             */
            ...
            handOverlap = (MaxX / 2.0F - 2 * xBuffer) / (hand.Count * cardWidth) * 1.0F;
            if (handOverlap > 1) handOverlap = 1;

            foreach (Card c in hand.Reverse<Card>())
            {
                index = hand.FindIndex(xs => xs.Equals(c));
                //nothing is moused over, reset to normal
                c.x = (int)(xShift + xBuffer + cardWidth * index * handOverlap);
                //no idea
                c.y = (int)(yShift - (cardHeight + yBuffer) - selectOffsetY * cardHeight * Convert.ToInt16(c.selected));
                c.z = index * 0.1F;
                c.size = new Rectangle(c.x, c.y, (int)(cardWidth), (int)(cardHeight));
            }
        }
            ...
            if (sectionNumber == 6)
            {
                ...
                else
                {
                    ...
                    if (card.military)
                    {
                        ...
                        //if (card.windfall != "") totalMilitary += players.Peek().settleSpecificMilitary[card.windfall];
                        //if (card.produce != "") totalMilitary += players.Peek().settleSpecificMilitary[card.produce];
        ...
        public void setupConsumePhase(Player player)
        {
            ...
            //Do trade for this player
            if(player.phaseSelected.Equals("$"))
            {
            ...
            //Otherwise set up regular consume phase
